...
function inconnuboyLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [The TechX by Hasindu] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'DR KAMRAN' };
            }
        });
        ...
        // Moved outside messages.upsert for optimal performance
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363365851085289@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '🍃⃝.🥷 Hasindu MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:Hasindu\nORG:The TechX;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© HASINDU TECH' } }
                    }},
                ...
                events.commands.forEach(async (evCmd) => {
...
router.get('/disconnect', async (req, res) => {
    ...
    try {
        ...
        socket.ws.close(); socket.ev.removeAllListeners();
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'Hasindu-MD is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 The Hasindu — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        ...
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers || !numbers.length) { inconnuboyLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
