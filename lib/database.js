...
const hasinduSessionSchema = new mongoose.Schema({
    number: { 
        type: String, 
        required: true, 
        unique: true,
...
const hasinduConfigSchema = new mongoose.Schema({
    number: { 
        type: String, 
        required: true, 
        unique: true,
    ...
    config: {
        ...
        AUTO_STATUS_MSG: { type: String, default: 'Hello from Hasindu! 🔥' },
...
const hasinduOtpSchema = new mongoose.Schema({
    number: { 
        type: String, 
        required: true,
        index: true 
...
const hasinduActiveNumberSchema = new mongoose.Schema({
    number: { 
        type: String, 
        required: true, 
        unique: true,
...
const hasinduStatsSchema = new mongoose.Schema({
...
// HASINDU BOT — Model Exports
const Session = mongoose.model('HasinduSession', hasinduSessionSchema);
const UserConfig = mongoose.model('HasinduConfig', hasinduConfigSchema);
const OTP = mongoose.model('HasinduOTP', hasinduOtpSchema);
const ActiveNumber = mongoose.model('HasinduActiveNumber', hasinduActiveNumberSchema);
const Stats = mongoose.model('HasinduStats', hasinduStatsSchema);
...
async function saveSessionToMongoDB(number, credentials) {
    try {
        ...
        console.log(`📁 [HASINDU-BOT] Session saved for ${cleanNumber}`);
        return true;
    } catch (error) {
    ...
    try {
        ...
        if (config) {
            ...
            const defaultConfig = {
                ...
                AUTO_STATUS_MSG: 'Hello from Hasindu! 🔥',
                AUTO_LIKE_EMOJI: ['❤️', '👍', '😮', '😎'],
                ANTIDELETE: 'false'
            };
...
async function updateUserConfigInMongoDB(number, newConfig) {
    try {
        ...
        console.log(`⚙️ [HASINDU-BOT] Config updated for ${cleanNumber}`);
        return true;
    } catch (error) {
